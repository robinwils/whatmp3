...
def copy_other(opts, flacdir, outdir):
	if opts.verbose: print('COPYING other files')
...
def replaygain(opts, codec, outdir):
	if opts.verbose:
		print("APPLYING replaygain " + codec)
...
def setup_parser():
	...
	p.add_argument('flacdirs', nargs='+', metavar='flacdir', help='directories to transcode')
	return p
...
def transcode(f, flacdir, mp3_dir, codec, opts, lock):
	...
	flac_cmd = "flac -sdc -- " + escape_percent(shlex.quote(f)) + " | " + flac_cmd
	flac_cmd = flac_cmd % {
...
def main():
	...
	for flacdir in opts.flacdirs:
		...
		flacfiles = []
		if not os.path.exists(opts.torrent_dir): os.makedirs(opts.torrent_dir)
		for dirpath, dirs, files in os.walk(flacdir, topdown=False):
			for name in files:
		...
		if opts.ignore and not flacfiles:
			if not opts.silent: print("SKIP (no flacs in): %s" % (os.path.relpath(flacdir)))
			continue
		if opts.original:
			if not opts.silent: print('BEGIN ORIGINAL FLAC')
			if opts.output and opts.tracker and not opts.notorrent:
				make_torrent(opts, flacdir)
			if not opts.silent: print('END ORIGINAL FLAC')
		...
		for codec in codecs:
			...
			if not opts.silent: print('END ' + codec + ': %s' % os.path.relpath(flacdir))
